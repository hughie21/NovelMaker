...
import { ref, reactive, watch, onMounted, provide, unref, h} from 'vue';
import { ElMessage, ElMessageBox, ElNotification, ClickOutside as vClickOutside  } from 'element-plus'
...
const fonts = utils.fonts;
const rowNum = ref(3)
const columnNum = ref(3)
const sizes = []
const fontSizeVal = ref(12)
const E = utils.editorRef;
...
const fontVal = utils.fontVal;
const headerVal = utils.headerVal;
...
const openFilePicker = async () => {
    ...
    utils.currentSave.value = res.Msg;
}
...
const saveFilePicker = async (saveAs) => {
    ...
    if (utils.currentSave.value !== "" && !saveAs) {
        FileSave(utils.currentSave.value, JSON.stringify(tempData), true).then((res)=> {
            if(res.Code === 0) {
                ElMessage.info(t('message.saveSuccess'));
                utils.change.value = false;
                utils.currentSave.value = res.Data;
            }else if(res.Code === -1){
                return;
            }else{
...
const changeBackColor = (val) => {
    const E = utils.editorRef.value;
    E.chain().focus().setTextBackColor(utils.rgbaToHex(val)).run();
}
...
const test = () => {
    const E = utils.editorRef.value;
    // console.log(E.state.tr)
    // console.log(E.chain().focus().setFontFamily('Bahnschrift').run())
    // colorPopper.value.show()
    const {doc, tr} = E.state;
    doc.descendants((node, pos) => {
        if (node.isText) {
            const textContent = node.text;
            const regex = new RegExp("成功", 'g');
            const index = textContent.search(regex);
            if(index != -1){
                E.chain().setTextSelection({from:pos, to:pos+2}).setTextBackColor("#bab815").run()
            }
        }
    })
    // console.log(utils.currentSave.value)
    // E.chain().setContent('<span style="display:flex;justify-content:left;"><img src="http://127.0.0.1:7288/0164b793d2f9252d.jpg" alt="0164b793d2f9252d.jpg" style="width:38%;"></span><p></p>').run();
}
                    ...
                    <div class="func_btn-list">
                        <div class="func_btn-container list_row">
                            <span class="btn-group" style="width:40px;">
                                <button class="el-button btn func_btn-big" @click="handlePaste">
                                    <i class="el-icon">
                            ...
                            <span class="btn-group">
                                <span style="margin-bottom: 2px;" >
                                    <el-tooltip :content="t('toolBar.tooltip.copy')" placement="top" effect='dark'>
                                        <button class="el-button btn is-link" @click="handleCopy">
                                            <i class="el-icon">
                                ...
                                <span>
                                    <el-tooltip :content="t('toolBar.tooltip.cut')" placement="bottom" effect='dark'>
                                        <button class="el-button btn is-link" @click="handleCut">
                                            <i class="el-icon">
                            ...
                            <span class="btn-group">
                                ...
                                <div style="display: flex; flex-direction:row; align-items: center; margin-left: 3px; margin-top: 5px">
                                    <el-tooltip :content="t('toolBar.tooltip.bold')" placement="bottom" effect='dark'>
                                        <el-button text size="small" @click="utils.editorRef.value.chain().focus().toggleBold().run()">
                                            <i class="el-icon">
                                    ...
                                    <el-tooltip :content="t('toolBar.tooltip.italic')" placement="bottom" effect='dark'>
                                        <el-button text size="small" @click="utils.editorRef.value.chain().focus().toggleItalic().run()">
                                            <i class="el-icon">
                                    ...
                                    <el-tooltip :content="t('toolBar.tooltip.strike')" placement="bottom" effect='dark'>
                                        <el-button text size="small" @click="utils.editorRef.value.chain().focus().toggleStrike().run()">
                                            <i class="el-icon">
                                    ...
                                    <el-tooltip :content="t('toolBar.tooltip.clean')" placement="bottom" effect='dark'>
                                        <el-button text size="small" @click="utils.editorRef.value.chain().focus().unsetAllMarks().run()">
                                            <i class="el-icon">
                            ...
                            <span class="btn-group">
                                <div style="display: flex; flex-direction:row; align-items: center;">
                                    ...
                                    <el-select v-model="fontSizeVal" size="small" style="width: 80px; margin-left: 3px" @change="handleFontSelChange">
                                        <el-option
                                          v-for="item in sizes"
                                          :key="item.value"
                                          :label="item.label"
